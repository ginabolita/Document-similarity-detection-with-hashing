...
vector<vector<int>> LSH2(const vector<int> &signature1, const int &b)
{
    ...
    for (int i = 0; i < b; i++)
    {
        ...
        //  !!! IMPORTANTE: a todos los subvectores 1 les aplicamos la misma funcion hash a los 2 otras distinta ....
        //                   para despues al comparar el 1 de sig1 con el 1 de sig2, el 2 de sig1 con el 2 de sig2, etc
        for (int j = 0; j < subSignatures[i].size(); j++)
        {
            // ENTIENDO QUE HE APLICADO BIEN EL HASH?
            subSignatures[i][j] = (hashCoefficients[i].first * subSignatures[i][j] + hashCoefficients[i].second)%p;
        }
    }
...
bool LSH(const vector<int> &signature1, const vector<int> &signature2, const int &b)
{
    // Divide Signature into b sub signatures
...
int main(int argc, char *argv[])
{
   ...
   // cout << "Número de k-shingles en texto 2: " << KT2.size() << endl;
    ...
    bool similar =  LSH(signature1, signature2, stoi(argv[4]));
    if (similar)
    {
